...
- Individually cached embeddings built an `EmbeddingsResponse` with an int token count, which this fork types as `Usage`. A fully cached response now reports no usage, and a partially cached one reports the usage of the portion actually generated.

### Notes

- Fork-specific behaviour preserved through the merge: per-modality `Usage`, `EmbeddingsResponse::$usage`, the Images API usage attribution, and the omission of `response_format` on image requests.
